...
public abstract class BaseController implements Serializable {
    ...
    public void logInnerField(Logger logger, Object instance) {
        if(instance==null) {
            logger.error("Nessun campo da mostrare.");
        }
        else {
            Field[] fList = instance.getClass().getDeclaredFields();
            for(Field f : fList) {
                if(f.getAnnotations().length==0) {
                    f.setAccessible(true);
                    try {
                        logger.error("{}: [{}]", f.getName(),  String.valueOf(f.get(instance)));
                    }
                    catch(IllegalAccessException iae) {
                        logger.error("{}: Impossibile accedere alla proprietà a causa di: {}", f.getName(), iae);
                    }
                }
            }
        }
